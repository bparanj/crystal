...
`uuid` is used if you prefer UUID generation, but we’ll stick to 4-digit codes as before.
...
The frontend code remains the same as previously. It communicates with `/api/save` and `/api/load` just as before.
